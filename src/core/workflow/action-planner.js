...
function findCandidateBySelector(candidates, selector) {
...
function scoreSubmitCandidate(candidate, inputCandidate) {
  const tag = String(candidate.tag || '').toLowerCase();
  const type = String(candidate.type || '').toLowerCase();
  const role = String(candidate.role || '').toLowerCase();
  const text = candidateSearchText(candidate);
  const area = candidateArea(candidate);
  const inputIndex = Number(inputCandidate?.index || 0);
...
function findSubmitCandidate(candidates, inputCandidate) {
  if (!inputCandidate) {
    return null;
  }
...
function createNoopPlan(reason, detail = {}) {
  return {
    type: 'none',
    reason,
    detail,
...
function createPausePlan(reason, detail = {}) {
  return {
    type: 'pause',
...
function toInputCandidatePayload(candidate) {
  if (!candidate) {
    return null;
  }
  return {
...
function toSubmitCandidatePayload(candidate) {
  if (!candidate) {
    return null;
  }
  return {
...
function createClickPlan(input = {}) {
  return {
    type: 'click_candidate',
    stage: input.stage,
    selector: input.candidate.selector,
    candidate: toSubmitCandidatePayload(input.candidate),
    terminalOutcome: input.terminalOutcome || '',
  };
}

function createSelectPlan(input = {}) {
  const selection = String(input.selection || '').trim();
  return {
    type: 'select_option',
    stage: input.stage,
    selector: input.candidate.selector,
    candidate: toInputCandidatePayload(input.candidate),
    selection,
    terminalOutcome: input.terminalOutcome || '',
  };
}

function createDeliveryPlan(input = {}) {
  return {
    type: 'select_delivery_and_submit',
    stage: input.stage,
    selection: input.selection,
    optionSelector: input.optionCandidate.selector,
    optionCandidate: toInputCandidatePayload(input.optionCandidate),
    submitSelector: input.submitCandidate?.selector || '',
    submitCandidate: toSubmitCandidatePayload(input.submitCandidate),
    terminalOutcome: 'need_otp',
  };
}
...
function createFillPlan(input = {}) {
  ...
  return {
    ...
    inputCandidate: toInputCandidatePayload(input.inputCandidate),
...
function candidateMatchesDeliverySelection(candidate = {}, selection = '') {
  const wanted = String(selection || '').trim().toLowerCase();
  if (!wanted) {
    return false;
  }
...
function scoreDeliveryOption(candidate, selection) {
  const tag = String(candidate.tag || '').toLowerCase();
  const type = String(candidate.type || '').toLowerCase();
  const text = candidateSearchText(candidate);
  let score = 0;
...
function findDeliveryOptionCandidate(candidates, selection) {
  return candidates
...
function findDeliverySubmitCandidate(candidates, optionCandidate) {
  const optionIndex = Number(optionCandidate?.index || 0);
  return candidates
    .filter(item => item.visible === true && isButtonLike(item))
    .map(item => {
      const text = candidateSearchText(item);
      let score = scoreSubmitCandidate(item, optionCandidate);
...
function planRuntimeAction(input = {}) {
  ...
  if (stage.state === 'otp_delivery_selection') {
    const selection = String(
      payload.OTP_DELIVERY_SELECTION || payload.OTP_SELECTION || ''
    ).trim();
    if (!selection) {
    ...
    if (String(deliveryCandidate.tag || '').toLowerCase() === 'select') {
      const options = Array.isArray(deliveryCandidate.options)
        ? deliveryCandidate.options
        : [];
      const matched = options.find(option => optionMatchesSelection(option, selection));
      if (!matched) {
